...
def init_backup(master_dir, backup_dir, exclusions, mounted_volume):
    ...
    # Verify and prepare the backup directory
    verify_and_create_backup_dir(backup_dir, mounted_volume)
...
def incremental_backup(master_dir, backup_dir, exclusions, mounted_volume):
    """Incremental backup."""
    date_time = datetime.now().strftime('%Y-%m-%d-%H%M')
    new_backup = os.path.join(backup_dir, date_time)
    current_backup = os.path.join(backup_dir, 'current')
...
def main():
    ...
    mounted_volume = config.get('backup_dir_is_mounted_volume', False)
    ...
    if mode == 'init':
        init_backup(master_dir, backup_dir, exclusions, mounted_volume)

    elif mode == 'run':
        incremental_backup(master_dir, backup_dir, exclusions, mounted_volume)
